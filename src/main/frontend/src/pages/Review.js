...
const Review = () => {
  ...
  const renderThisPage = async(page) => {
    const getInfo = await ReviewAPI.GetReview(festivalId, page)
        .then((result) => {
  ...
  const onClickPreviousPage = async () => {
    if (currentPage > 0) {
      const getPreviousPage = await ReviewAPI.GetReview(festivalId, currentPage - 1)
          .then((result) => {
            if (result.status === 200) {
              console.log(result.data);
              console.log(result.data.content);
  ...
  const onClickNextPage = async () => {
    if (currentPage + 1 < totalPages) {
      const getNextPage = await ReviewAPI.GetReview(festivalId, currentPage + 1)
          .then((result) => {
            if (result.status === 200) {
              console.log(result.data);
              console.log(result.data.content);
  ...
  const getReviewList = async () => {
    const getInfo = await ReviewAPI.GetReview(festivalId,currentPage)
        .then((result) => {
          if (result.status === 200) {
            console.log(result.data);
            console.log(result.data.content);
